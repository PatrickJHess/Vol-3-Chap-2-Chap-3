...
The *Bond Pay Data* notebook then uses <font color='green'>adust_bond_pay_dates()</font> to convert scheduled payment dates to actual payment dates. Scheduled payment dates are first generated by counting backward from the maturity date and are stored in a Pandas Series. The Pandas <font color='green'>apply()</font> method is then used with <font color='green'>adust_bond_pay_dates()</font>  to convert these scheduled dates into the actual payment dates.
